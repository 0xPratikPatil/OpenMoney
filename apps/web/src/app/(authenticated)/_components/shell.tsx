...
export function Shell({ session, children }: ShellProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      <aside className="flex flex-col h-full shrink-0 border-r border-border transition-all duration-200" style={{ width: SIDEBAR_W }}>
        ...
        <nav className="flex-1 overflow-y-auto py-2 px-1.5 space-y-4">
          {NAV_GROUPS.map((group) => (
            <div key={group.label}>
              {!collapsed && <p className="px-2 mb-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">{group.label}</p>}
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button key={item.id} onClick={() => router.push(item.href)}
                      className={`relative flex items-center w-full rounded-md transition-colors duration-100 ${collapsed ? 'justify-center h-8 w-8 mx-auto' : 'gap-2.5 px-2.5 h-8'} ${isActive ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-accent/50'}`}>
        ...
        <div className="p-1.5 border-t border-border">
          <button onClick={() => setCollapsed(!collapsed)} className="flex items-center justify-center w-full h-7 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors">
        ...
        <header className="flex items-center justify-between h-10 px-4 shrink-0 border-b border-border">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 pl-2 border-l border-border">
              <div className="flex items-center justify-center w-6 h-6 rounded-md bg-accent text-accent-foreground text-[10px] font-mono font-semibold">
                {session?.user?.name?.[0] || '?'}
              </div>
              <span className="font-mono text-[11px] text-muted-foreground hidden sm:block max-w-[100px] truncate">{session?.user?.name || session?.user?.email}</span>
              <Link href="/api/auth/sign-out" className="p-1 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors" title="Sign out">
                <LogOut size={13} />
              </Link>
            </div>
          </div>
