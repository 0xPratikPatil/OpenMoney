/**
 * Index (market indices) Domain Router
 *
 * Routes for market index data:
...
/**
 * GET /api/index/historical
 * Get historical index price data.
 *
 * Query params:
 *   symbol   - Index symbol (e.g., "^GSPC") (required)
...
router.get(
  ...
  async (c) => {
    ...
    return createProviderQueryHandler(c, executor, provider, "index/historical", {
...
router.get(
  ...
  async (c) => {
    ...
    return createProviderQueryHandler(c, executor, provider, "index/available", {});
  },
);
