/**
 * Futures Domain Router
 *
 * Routes for futures data:
 *   historical, curve
 *
...
/**
 * GET /api/futures/historical
 * Get historical futures price data.
 *
 * Query params:
 *   symbol   - Futures symbol (e.g., "ES=F") (required)
...
router.get(
  ...
  async (c) => {
    ...
    return createProviderQueryHandler(c, executor, provider, "futures/historical", {
...
router.get(
  ...
  async (c) => {
    ...
    return createProviderQueryHandler(c, executor, provider, "futures/curve", { symbol });
  },
);

export { router as futuresRouter };
